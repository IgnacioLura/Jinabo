    ...
    <div className="px-4 md:px-8 py-6 max-w-5xl mx-auto">
      ...
      <motion.div initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }}>
        <Link
          href="/articulos"
          scroll={false}
          onClick={() => sessionStorage.setItem("articulos-from-detail", "1")}
          className="text-sm text-[var(--foreground)]/50 hover:text-[var(--brand)] transition-colors inline-flex items-center gap-1"
        >
          <ArrowLeft size={14} />
          Volver
        </Link>
